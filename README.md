...
このパッケージは、Flutterアプリケーションで使用できる便利なウィジェットを提供します。
主にプライベートプロジェクトでサンプル作成の際に使用することをターゲットにしています。

## 特徴

- `ActionBottomSheet`: カスタマイズ可能なボトムシートウィジェット。
- `CustomSnackBar`: カスタマイズ可能なスナックバーウィジェット。
- `PlatformDialog`: プラットフォームに応じたダイアログウィジェット。

## 始めに

このパッケージを使用するには、`pubspec.yaml` に以下を追加してください。
